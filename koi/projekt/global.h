...
enum type { INTEGER, STRING, TYPE_ERROR, VOID };
...
extern void error(const char* m);  /*  generates all error messages  */
...
extern node* mkleaf(int, int, char*); /* creates a new leaf node in the syntax tree */
...
enum type type_checker(node *root); /*analyzes the type of all the expressions in the syntaxtree*/