...
node* mkleaf(int node_type, int leaf_value, char *str)
{
	//debug
    ...
    if(node_type == NUM)
        ...
        new_node->var_type = STRING;
    else if(node_type == ID)
        new_node->var_type = symtable[leaf_value].var_type;
...
void printtree1(node* root, int level) {
	...
	;
		...
		printf("%*s", 2*level, "");
		printf("%s %s\n", symtable[root->leaf_value].var_type == INTEGER ? "int" : "str", symtable[root->leaf_value].lexptr);
	}
	else if (root->node_type == NUM) {
		printf("%*s", 2*level, "");
		printf("%d\n", root->leaf_value);
	}
    else if (root->node_type == STR) {
		printf("%*s", 2*level, "");
		printf("%s\n", root->str);
	}
